...
def run_vmax_final_master(temp, p_init, duration, h2o_lpm, grain_init, jdd_theta, cement_depth, ld_ratio, jdd_type):
    # Error Prevention: Ensure variables are never zero or negative
    duration = max(1, duration)
    p_init = max(1.0, p_init)
    
    t_steps = np.arange(0, duration + 1, 1)
    
    # 1. Prediction: Gamma-DNA & Mach Number (Target γ ≈ 1.21) [cite: 11]
    gamma_eff = max(1.05, 1.38 - (temp / 12500) + (ld_ratio * 0.004))
    R_spec = 518.6 
    ...
    # 3. Prediction: Acoustic Load (Lighthill's Law) - FIXING VALUEERROR 
    # Guarding against division by zero in (gamma_eff - 1)
    v_term = (2 * gamma_eff * R_spec * temp) / max(0.001, gamma_eff - 1)
    p_ratio = (1.05 / p_decay)**((gamma_eff - 1) / gamma_eff)
    v_exit = np.sqrt(max(0, v_term * (1 - p_ratio)))
    acoustic_db = 120 + 10 * np.log10(max(1, (v_exit**8) / 1e12))
    
    # 4. Prediction: JDD Sustainability (Transpiration) 
    theta_rad = np.radians(jdd_theta)
    thermal_lag = 1 / (1 + (0.015 * cement_depth)) # 100mm refractory lag [cite: 14]
...
st.markdown("**Developer:** R. Puneesh kumar | **System:** Dynamic Decision Intelligence")
...
with st.sidebar:
    ...
    st.header("2. Hardware Sustainability")
...
with a2: st.plotly_chart(go.Figure(go.Scatter(x=df['Sec'], y=df['Acoustic Load (dB)'], line=dict(color='#00d4ff', width=3))).update_layout(title="Acoustics vs Time", template="plotly_dark"))
